...
\end{equation}
where $ c_m $ is the constant representing the prediction for all samples belonging to the m-th terminal region $ R_m $.
Assuming that the loss is measured with an l2-metric, the predicted constant $ \hat{c}_m $ for each terminal region $ R_m $ is the average of all $ y $ for each $ x \in R_m $:
\begin{equation}
...
To find $ j $ and $ s $ that minimize the overall loss for both terminal regions $ R_1 $ and $ R_2 $, the greedy algorithm initially solves

\begin{equation}\label{minjs}
	\min_{j,s} \bigg [\min_{\hat{c}_1} \sum_{x_i \in R_1(j,s)} L(y_i, \hat{c}_1) + \min_{\hat{c}_2} \sum_{x_i \in R_{2}(j,s)} L(y_i, \hat{c}_{2})].
\end{equation}
